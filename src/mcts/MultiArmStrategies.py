class MultiArmStrategies:
    ...
    def multiarm_strategy(self, node, used_features, scoring_function, other_scores, params):
        ...
        if self._name == 'continuous':
            return self._continuous_strategy(node, used_features, scoring_function, other_scores)
        elif self._name == 'discrete':
            return self._discrete_strategy(node, used_features, scoring_function, other_scores)
        else:
            raise Exception("Error getting multiarm strategy, strategy \'" + self._name + "\' is not supported.")   
    
    def _continuous_strategy(self, node, used_features, scoring_function, other_scores):
        if(len(node.child_nodes) == 0):
            self._add_all_child_nodes(node, used_features)
            return node.child_nodes[0]
        else:
            return _get_best_node(self, node, scoring_function, other_scores)

    def _discrete_strategy(self, node, used_features, scoring_function, other_scores):
        if len(node.child_nodes) == 0 or self._should_add_child(node):
            return self._add_child_node(node, other_scores)
        else:
            return _get_best_node(self, node, scoring_function, other_scores)
    ...
    def _get_best_node(self, node, scoring_function, other_scores):
        best_score = 0
        best_node = None
        tmp_score = 0
    ...
    def _add_child_node(self, node, other_scores):
        best_score = 0
        best_feature = None
        
        for feature in (self._all_node_names - used_features):
            curr_score = other_scores.get_g_rave_score(feature)
